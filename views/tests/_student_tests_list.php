    ...
    <div class="col-sm-12">
        <div class="row flex-list" style="margin-bottom: 15px">
            <div style="max-width:250px;">
                <select class="watch-select single-select form-control" ng-model="current_filter" ng-change='loadTests()'>
                    <option value=''  data-subtext="{{ counts.student_tests.all || '' }}">все тесты</option>
                    <option disabled>───────</option>
                    <option value='not_started'  data-subtext="{{ counts.student_tests.not_started || '' }}">не приступали</option>
                    <option value='in_process'  data-subtext="{{ counts.student_tests.in_process || '' }}">в процессе</option>
                    <option value='finished'  data-subtext="{{ counts.student_tests.finished || '' }}">пройденные</option>
                </select>
            </div>
        </div>
        ...
        <table class="table table-hover vertical-align-table">
            <tr ng-repeat="StudentTest in StudentTests">
                ...
                <td width="220">
                    <a href="tests/edit/{{ StudentTest.id_test }}" target="_blank">{{getStudentTest(StudentTest.id_test).name}}</a>
                </td>
                <td width="120">
                ...
                <td>
                    ...
                    <span ng-show="StudentTest.inProgress">в процессе, осталось {{ timeLeft(StudentTest) }}</span>
                </td>
                <td>
                    <div ng-show='testDisplay(StudentTest)'>
						<span ng-repeat="Problem in getStudentTest(StudentTest.id_test).Problems">
							<span class="circle-default {{ getStudentAnswer(Problem, StudentTest) }}" title="{{ getTestHint(Problem, StudentTest) }}"></span>
						</span>
                    </div>
                </td>
                <td>
                    <span ng-show='testDisplay(StudentTest)'>набрано {{ getCurrentScore(getStudentTest(StudentTest.id_test), StudentTest) }} из {{ getStudentTest(StudentTest.id_test).max_score }} баллов</span>
                </td>
                <td>
