...
<p>1.3. Продолжительность образовательной программы по программе курса
	<span ng-repeat="program in contract.subjects">
			«{{SubjectsFull2[program.id_subject]}}-{{ contract.info.grade == <?= Grades::EXTERNAL ?> ? 'Э' : contract.info.grade }}-{{ program.count }}» ({{ ceil(program.count * 3) }}
...
<p>3.1. Общая стоимость Услуг Исполнителя по Договору складывается из стоимости {{ subjectCount(contract) }} <ng-pluralize count="subjectCount(contract)" when="{
   'one'	: 'занятия',
   'few'	: 'занятий',
   'many'	: 'занятий',
...
<p>11.2.  Настоящий Договор вступает в силу с даты, его подписания, заключен на срок до 01 июля {{ contract.info.year + 1 }} года. </p>
