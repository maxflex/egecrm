...
//			error_reporting(E_ALL);
		...
		public function actionTransferOne()
		{
			$GroupTeacherStatuses = GroupTeacherStatuses::findAll();
			
			foreach ($GroupTeacherStatuses as $G) {
				$Group = Group::findById($G->id_group);
				
				GroupAgreement::add([
					"id_entity" 	=> $G->id_teacher,
					"type_entity"	=> "TEACHER",
					"id_group"		=> $G->id_group,
					"day_and_time"	=> self::_cleanDateAndTime($Group->day_and_time),
					"id_status"		=> $G->id_status,
				]);
			}
		}
		
		public function actionTransferTwo()
		{
			$GroupStudentStatuses = GroupStudentStatuses::findAll();
			
			foreach ($GroupStudentStatuses as $G) {
				$Group = Group::findById($G->id_group);
				
				GroupAgreement::add([
					"id_entity" 	=> $G->id_student,
					"type_entity"	=> "STUDENT",
					"id_group"		=> $G->id_group,
					"day_and_time"	=> self::_cleanDateAndTime($Group->day_and_time),
					"id_status"		=> $G->id_status,
		...
		public function actionTransferSms()
		{
			$GroupStudentStatuses = GroupStudentStatuses::findAll([
			...
			foreach ($GroupStudentStatuses as $G) {
				...
				if (!$Group->id_branch || !$Group->id_subject || !$Group->first_schedule || !$Group->cabinet) {
					continue;
				}
				...
				GroupSms::add([
					...
					"first_schedule"	=> $Group->first_schedule,
		...
		public function actionTransferLikes()
		{
			$GroupStudentStatuses = GroupStudentStatuses::findAll([
			...
			foreach ($GroupStudentStatuses as $G) {
				...
				GroupTeacherLike::add([
					"id_student" 		=> $G->id_student,
					"id_teacher"		=> $Group->id_teacher,
					"id_status"			=> $G->review_status,
				]);
			}
		}
