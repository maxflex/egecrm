<div ng-app="Request" ng-controller="ListCtrl" ng-init="<?= angInit("requests", $Requests) ?>">
		...
		<div class="col-sm-12">
			<ul class="nav nav-tabs" ng-init="<?= angInit("request_statuses", RequestStatuses::$all) ?>">
				<li ng-class="{'active' : $index == 0}" ng-repeat="(key, value) in request_statuses">
					<a href="#{{key}}" ng-click="changeList(key)" data-toggle="tab" aria-expanded="{{$index == 0}}">{{value}}</a></li>
			</ul>
		</div>
	</div>
