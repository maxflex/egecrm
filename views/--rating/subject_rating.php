<div ng-app="Rating" ng-controller="MainCtrl" ng-init="<?= $ang_init_data ?>">
	<div class="top-links">
		<a ng-repeat="(id_subject, name) in Subjects" href="rating/subject/{{id_subject}}"
			ng-class="{'active': id_subject == <?= $id_subject ?>}">{{name}}</a>
	</div>
	<div class="top-links">
	...
	<table class="table table-hover">
		...
		<tbody>
			<?php foreach (Branches::$all as $id_branch => $name): ?>
				<tr>
					<td onclick="redirect('rating/<?= $id_branch ?>')" class="pointer"><?= Branches::getName($id_branch) ?></td>
					<?php for ($i = 9; $i <= 11; $i++): ?>
						<td>
							<span class="score-field">
								<span ng-show="data['<?= $id_branch ?>']['<?= $i ?>']['<?= $id_subject ?>']">
									{{data['<?= $id_branch ?>']['<?= $i ?>']['<?= $id_subject ?>']}}
								</span>
							</span>
							<span ng-repeat="branch_load in BranchLoad[<?= $i ?>][<?= $id_branch ?>] track by $index" 
								<?php if (in_array(User::fromSession()->id, RatingController::$CAN_EDIT_BRANCHLOAD)): ?>
									ng-click="changeLoadSubject(<?= $id_branch ?>, <?= $i ?>, <?= $id_subject ?>, $index)"
								<?php endif ?>>
								...
								<svg xmlns="http://www.w3.org/2000/svg" version="1.1" class="svg-metro rating rating-add" 
									ng-hide="BranchLoad[<?= $i ?>][<?= $id_branch ?>].length >= 4">
				            		<circle fill="transparent" r="6" cx="7" cy="7" stroke="#C0C0C0" stroke-width="1" 
				            			ng-click="addLoadSubject(<?= $id_branch ?>, <?= $i ?>, <?= $id_subject ?>)"></circle>
								</svg>
								<?php endif ?>
						</td>
					<?php endfor ?>
			...
			<?php endforeach ?>
			<tr onclick="redirect('rating/sum')" class="pointer">
				<td>в сумме по филиалам</td>
				<?php for ($i = 9; $i <= 11; $i++): ?>
					<td><?= $sumRating[$i][$id_subject] ?></td>
				<?php endfor ?>
			</tr>
		</tbody>
	</table>
