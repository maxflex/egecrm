...
<div class="lightbox-new lightbox-additional-lesson">
	<h4 style="display: inline-block">{{new_additional_lesson.id ? "Редактировать" : "Добавить"}} занятие</h4>
	<div class="form-group">
		<div class="form-group">
			<?= Subjects::buildSelector(false, false, [
				"ng-model" => "new_additional_lesson.id_subject",
			], true) ?>
		</div>
		<div class="form-group">
			<?= Grades::buildSelector(false, false, ["ng-model" => "new_additional_lesson.grade"]) ?>
		</div>
		<div class="form-group">
			<input class="form-control digits-only"
				ng-model="new_additional_lesson.teacher_price"  placeholder="цена преподавателя">
		</div>
		<div class="form-group">
            <select class="form-control" ng-model="new_additional_lesson.year" style='margin: 0; width: 100%'>
		...
		<div class="form-group">
			<input style='margin: 0' placeholder="дата" class="form-control bs-date full-width" id="add-payment-date" ng-model="new_additional_lesson.date">
		</div>
		<div class="form-group">
			<textarea maxlength="255" placeholder="назначение" class="form-control full-width" id="add-payment-purpose" ng-model="new_additional_lesson.purpose"></textarea>
		</div>
	</div>
	<center>
		<button class="btn btn-primary ajax-payment-button full-width" ng-click="addAdditionalPayment()">{{new_additional_lesson.id ? "Редактировать" : "Добавить"}}</button>
		<button class="btn btn-primary btn-danger full-width" ng-show="new_additional_lesson.id"
