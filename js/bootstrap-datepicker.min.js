	...
	j.prototype = {
		...
		getClassNames: function(b) {
			var c = [],
				...
				g = new Date;
        if(!ang_scope.cancelled_lesson_dates)ang_scope.cancelled_lesson_dates=[];
			return b.getUTCFullYear() < d || b.getUTCFullYear() === d && b.getUTCMonth() < f ? c.push("old") : (b.getUTCFullYear() > d || b.getUTCFullYear() === d && b.getUTCMonth() > f) && c.push("new"), this.focusDate && b.valueOf() === this.focusDate.valueOf() && c.push("focused"), this.o.todayHighlight && b.getUTCFullYear() === g.getFullYear() && b.getUTCMonth() === g.getMonth() && b.getUTCDate() === g.getDate() && c.push("today"), -1 !== this.dates.contains(b) && -1===ang_scope.cancelled_lesson_dates.indexOf(moment(b).format("YYYY-MM-DD")) && c.push("active"), -1!==ang_scope.cancelled_lesson_dates.indexOf(moment(b).format("YYYY-MM-DD")) && c.push("cancelled"), (b.valueOf() < this.o.startDate || b.valueOf() > this.o.endDate || -1 !== a.inArray(b.getUTCDay(), this.o.daysOfWeekDisabled)) && c.push("disabled"), this.o.datesDisabled.length > 0 && a.grep(this.o.datesDisabled, function(a) {
				return e(b, a)
      ...
      case "td":
        ...
        this._setDate(c(d, e, f),undefined,g));

        /**
         *   g - clicked elem
         *   b - datapicker obj
         *
         *   дата удаляется из this.dates через один клик. чтобы добавить третое состояние
         *   придется добавить дату в this.dates еще раз, когда дата была активной.
         *
         *   а также на всякий случай проверяем классы. хотя это уже не нужно, т. к.
         *   там где классы автоматически добавляюся исправил чтобы учитовалось третое состояние.
         *
         *   g.hasClass("disabled") - все прошедщее занятия не должны меняться
         **/
        if(!g.hasClass("disabled")) {
          updatedDate = $(b.currentTarget).find('td').filter(function() {return $(this).text() === g.text()});
          if (g.hasClass('active')) {
            updatedDate.removeClass('active cancelled').addClass('cancelled');
            this.dates.push(this.viewDate);
          }
          else if (g.hasClass('cancelled')) {
            updatedDate.removeClass('cancelled active');
          }
        }
			}
		...
		_setDate: function(a, b, g) {
			clicked_date = a;
      /* если дата была активной то добавим ее на отмененные даты */
      /* если дата была отмененной то удалим ее из отмененных дат */
      if (g){
          if (g.hasClass("active")) ang_scope.cancelled_lesson_dates.push(moment(a).format("YYYY-MM-DD"));
          if (g.hasClass("cancelled")) ang_scope.cancelled_lesson_dates.splice(ang_scope.cancelled_lesson_dates.indexOf(moment(a).format("YYYY-MM-DD")), 1);
      }
