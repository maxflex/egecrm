<?php
	class Group extends Model
	{
		...
		public function __construct($array)
		{
			...
			$this->schedule_is_finished = $this->scheduleIsFinished();
		...
		public function scheduleIsFinished()
		{
			$allSchedule = GroupSchedule::findAll([
				'condition' => 'id_group =' . $this->id
			]);

			$schedule_is_finished = true;
			if ($allSchedule == false) {
				$schedule_is_finished = false;
			}

			foreach ($allSchedule as $schedule) {
				$visitJournal = VisitJournal::find([
					'condition' => 'id_group=' . $this->id . ' AND lesson_date="' . $schedule->date . '"',
				]);
				if (!$visitJournal) {
					$schedule_is_finished = false;
					break;
