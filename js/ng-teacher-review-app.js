...
angular.module("TeacherReview", ["ui.bootstrap"]).filter('range', function() {
  ...
  $scope.toggleUser = function(user_id) {
    user_id = $scope.Student.id_user_review === $scope.user.id ? 0 : $scope.user.id;
    return $.post("ajax/updateStudentReviewUser", {
      'student_id': $scope.Student.id,
      'user_id': user_id
    }, function() {
      var Review, j, len, ref, user, user_data;
      user = _.findWhere($scope.users, {
        id: user_id
      });
      user_data = {
        id_user_review: user.id,
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        Review = ref[j];
        _.extend(Review.Student, user_data);
      }
      return $scope.$apply();
    });
  };
  ...
  $scope.refreshCounts = function() {
    $timeout(function() {
      return $('.watch-select option').each(function(index, el) {
