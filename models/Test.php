<?php
	...
	class TestProblem extends Model
	{
		...
		public static function getCorrectAnswers()
		{
			$query = dbConnection()->query("
				select id_test, group_concat(id, '-', correct_answer) as problem_id_correct_answer from test_problems
				group by id_test
			");
			$return = [];
			while ($row = $query->fetch_object()) {
				$correct_answers = [];
				foreach(explode(',', $row->problem_id_correct_answer) as $e) {
					list($problem_id, $correct_answer) = explode('-', $e);
					$correct_answers[$problem_id] = $correct_answer;
				}
				$return[$row->id_test] = $correct_answers;
			}
			return $return;
		}
	...
	class TestStudent extends Model
	{
        ...
        public static function getData($page)
        {
            ...
            return [
                'data' 	=> $data,
                'item_count' => static::_count($search),
            ];
        }
