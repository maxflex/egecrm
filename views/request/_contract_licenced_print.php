...
<p>1.1. По настоящему Договору Исполнитель обязуется зачислить Обучающегося на обучение и оказать Обучающемуся образовательные услуги по программе дополнительного обучения по курсу (курсам): <span ng-repeat="subject in getSubjects(contract)">«{{SubjectsFull2[subject.id_subject]}}» ({{ subject.count_program }} <ng-pluralize count="subject.count_program" when="{
    'one' 	: 'занятие',
    'few'	: 'занятия',
    'many'	: 'занятий',
}"></ng-pluralize>){{!$last ? ", " : ""}}</span> (далее – «Услуги»), а Заказчик обязуется оплатить указанные Услуги в порядке и на условиях, которые установлены настоящим Договором.</p>
...
<p>1.5. Вид образования: дополнительное образование детей и взрослых, продолжительность обучения: 33 учебных недели. </p>
...
<p>Если Заказчик отказывается от исполнения Договора, то он оплачивает оказанные услуги (состоявшиеся занятия) к моменту отказа от договора).</p>
