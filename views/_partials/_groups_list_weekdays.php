...
<div class="ng-hide" ng-show="search.time && search.time.length">
	<div ng-repeat="weekday in weekdays" ng-show="inGroupDays($index)">
	<h4>{{ weekday.full }}</h4>
	<table class="table table-divlike" style="position: relative">
		<tr ng-repeat="Group in Groups | filter:groupsFilter" 
			ng-class="{
				'students-10': Group.students.length == 10,
				'students-11': Group.students.length == 11,
			}"
			class="group-list" data-id="{{Group.id}}" ng-show="inGroupDay($parent.$index, Group)">
			<td width="100">
