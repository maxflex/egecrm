...
<div id="contract-print-{{contract.id}}" class="printable">
	...
	<h4 style="margin-top: 0">№ {{id_request}}/{{contracts.length - $index}}</h4>
	...
	<p><b>Индивидуальный предприниматель Капралов Константин Александрович</b>, действующий на основании Свидетельства о государственной регистрации физического лица в качестве индивидуального предпринимателя серии 62 № 002030621,выданного межрайонной инспекцией Федеральной налоговой службы № 1 по г. Рязани 18 июня 2009 года, именуемый в дальнейшем «Агент», осуществляющий свою деятельность под коммерческим наименованием «ЕГЭ-Центр», в лице представителя Ксендзовой Татьяны Николаевны, действующего на основании Доверенности № 1 от 10 февраля 2015 г., с одной стороны, и гр. {{contractPrintName(representative)}} именуемый в дальнейшем «Принципал», с другой стороны, именуемые в дальнейшем «Стороны», заключили настоящий договор, в дальнейшем «Договор», о нижеследующем:</p>
	
	
		<h4>1. ПРЕДМЕТ ДОГОВОРА</h4>
	
	
	<p>1.1. Агент обязуется совершить от своего имени и за счет Принципала действия по организации предметных консультаций, направленных на подготовку Принципала (лица, назначенного Принципалом - консультируемого) к сдаче ЕГЭ или ГИА, согласно следующим требованиям Принципала, указанным в Заявке № {{id_request}} от <?= dateFormat($Request->date, true) ?> года (далее по тексту также – «Заявка принципала»), являющейся неотъемлемой частью настоящего договора (приложение № 1):</p>
	...
	<p>- при организации консультационного процесса, объединить консультируемых в группы по 4-12 человек, в соответствии с предметами (направлениями) и уровнем индивидуальной подготовки; при этом формирование групп и перевод консультирующихся из одной группы в другую, осуществляются Агентом самостоятельно, в зависимости от конкретной текущей ситуации, в части наполнения групп и интенсивности консультационных занятий;</p>
	...
	<p>- заблаговременно уведомить Агента о невозможности посещения ранее назначенного занятия в связи с болезнью или по иной уважительной причине, в письменном виде с предоставлением подтверждающих документов;</p>
	...
	<p>4.1. За совершение порученных действий по настоящему договору Принципал выплачивает Агенту вознаграждение в размере {{contract.sum | number}} ({{numToText(contract.sum)}} <ng-pluralize count="contract.sum" when="{
			...
			'few'	: 'рубля',
			'many'	: 'рублей',
		}"></ng-pluralize>) руб., согласно расчету Агента, исходя из необходимого перечня предметов подготовки и количества занятий, необходимых по каждому из заявленных предметов. Принципал уплачивает Агенту вознаграждение в размере 50% , что составляет {{(contract.sum / 2) | number}} ({{numToText((contract.sum / 2))}} <ng-pluralize count="(contract.sum / 2)" when="{
			'one'	: 'рубль',
			'few'	: 'рубля',
			'many'	: 'рублей',
		}"></ng-pluralize>) руб. на условиях предоплаты непосредственно при заключении настоящего Договора;</p>
	<p>- принципал обязуется выплатить вторую часть суммы в размере {{(contract.sum / 2) | number}} ({{numToText((contract.sum / 2))}} <ng-pluralize count="(contract.sum / 2)" when="{
			...
			'few'	: 'рубля',
			'many'	: 'рублей',
		}"></ng-pluralize>) руб., до 15 января 2016 года.</p>
	...
	<p>Договор считается расторгнутым с момента подписания сторонами Акта о расторжении договора.</p>
	...
	<p style="margin-bottom: 20px"><b>Принципал: {{representative.last_name}} {{representative.first_name}} {{representative.middle_name}}</b>, паспорт серия: {{representative.passport.series}} № {{representative.passport.number}} , выдан: {{representative.issued_by}}, дата выдачи {{representative.date_issued}} зарегистрирован по адресу: {{representative.address}}, адрес для направления почтовой корреспонденции: 127549 г. Москва, ш. Алтуфьевское д.60 к.84 телефон: {{representative.phone}} e:mail: {{representative.email}}</p>
	
	<p style="margin-bottom: 20px"><b>Консультируемый: {{student.last_name}} {{student.first_name}} {{student.middle_name}}</b>, паспорт серия: 4514 № 804322 выдан: Отдел УФМС России по гор. Москве по району Алтуфьевский, дата выдачи 25.12.2014, код подразделения: 770-076, зарегистрирован по адресу: 127549 г. Москва, ш. Алтуфьевское д.60 к.84; адрес для направления почтовой корреспонденции: 127549 г. Москва, ш. Алтуфьевское д.60 к.84; конт. телефон: {{student.phone}}</p>
	
	<p style="margin-bottom: 20px"><b>Агент</b> _________________ Ксендзова Т.Н.</p>
	
	<p><b>Принципал</b> _________________ ____________________</p>
</div>